...
const DiscoverySection: React.FC<DiscoverySectionProps> = ({
  ...
  const getPlaceholderForQuestion = (questionText: string, type: 'qualitative' | 'quantitative'): string => {
    const qTextLower = questionText.toLowerCase();
    if (type === 'quantitative' || 
        qTextLower.includes("how many") || 
        qTextLower.includes("percentage") || 
        qTextLower.includes("value") ||
        qTextLower.includes("rate") ||
        qTextLower.includes("time") ||
        qTextLower.includes("cost") ||
        qTextLower.includes("number") ||
        qTextLower.includes("average") ||
        qTextLower.includes("(metric)")) {
      return "Enter metric value (e.g., 100, 25%, $50)";
    }
    return "Enter qualitative details, descriptions, or observations...";
  };
    ...
    <div className="mb-8 p-6 border border-gray-200 rounded-lg shadow-sm">
      ...
      {questions.map((item, index) => (
        <div key={item.questionId} className="mb-6 pb-4 border-b border-gray-100 last:border-b-0 last:pb-0">
          ...
          <Textarea
            ...
            placeholder={getPlaceholderForQuestion(item.questionText, questionType)}
          />
        </div>
      ))}
      <div className="mt-6 space-y-2">
        <Input
          ...
          id={`${moduleId}-${questionType}-custom-question-input`}
        />
...
const DiscoveryQuestionsTab: React.FC<TabProps> = ({ appState, setAppState }) => {
  ...
  const handleAnswerChange = useCallback((moduleId: string, type: 'qualitative' | 'quantitative', questionId: string, answer: string) => {
    setAppState(prev => {
      if (!prev.discoveryQuestions[moduleId]) return prev; // Should not happen
  ...
  return (
    <div className="p-6 bg-white shadow rounded-lg">
      <h2 className="text-xl font-semibold text-gray-800 mb-6">Discovery Questions for {ALL_MODULES.find(m=>m.id === selectedModuleId)?.name || selectedModuleId}</h2>
      <DiscoverySection
